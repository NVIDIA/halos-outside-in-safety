...
feed, start from [Deploy the Safety Core](/getting-started/safety-core).
...
- [Deploy AI Perception](/getting-started/inference-pipeline) covers camera selection, placement,
...
[Integration Guide](/integration) documents how those messages are mapped to the safety
...
<a id="installation-of-the-safety-core" />
...
<a id="safety-core-setup-docker-runtime" />
...
<a id="running-safety-core" />

### 2.1 Setup
<a id="setup" />

#### 2.1.1 Create shared log directory
<a id="create-shared-log-directory" />
...
container is expected, not a fault. See [Deployment Guide](/deployment#safety-core-logs) for
which file each component writes and when.
</Note>

#### 2.1.2 Runtime library dependencies (Debian installation only)
<a id="runtime-library-dependencies-debian-installation-only" />
...
<a id="create-the-sensor-configuration-file" />
...
[Deploy the HIL Closed Loop](/testing/hil/quickstart) does with the Isaac Sim container.
</Note>
...
instead of the intended type. See [SEI Configuration](/reference/sei-configuration).
</Note>
...
<a id="launch-control-of-safety-function-on-a-forklift" />
...
<a id="option-a-b-x86-64-aarch64-sdm-on-ccplex" />
...
<a id="option-c-aarch64-sdm-on-fsi" />
...
<a id="launch-proximity-monitoring-preview" />
...
<a id="quick-start-with-launch-hoisa-sh" />
...
- The event-mapping configuration for the selected application must exist and
  ...
  [Event Mapping Configuration](/reference/event-mapping-configuration).
...
- `nvpss.conf` must be present and readable (by default at
  `/opt/nvidia/psf/bin/nvpss.conf`). See [SEI Configuration](/reference/sei-configuration).
...
For the full launcher option reference, see [launch_hoisa.sh](/reference/launcher). For
the browser-based log dashboard, see [Log Viewer](/reference/log-viewer).

### 2.5 Clearing a Latched Safe State
<a id="clearing-a-latched-safe-state" />
<a id="safe-release" />
...
<a id="safety-core-configuration" />
...
<a id="safety-event-integrator-configuration" />
...
for a deployment. See [SEI Configuration](/reference/sei-configuration).

### 3.2 Event Mapping Configuration
<a id="event-mapping-configuration" />
...
ingestion pipeline. See [Runtime Profile](/reference/runtime-profile).

## 5. Halos Outside-In Safety Error Handling
<a id="halos-outside-in-safety-error-handling" />
...
[Deployment Guide](/deployment#safety-core-troubleshooting), which pairs symptoms with diagnosis and
resolution steps.

### 5.2 Kafka Connectivity Errors
<a id="kafka-connectivity-errors" />

#### 5.2.1 Initialization failures
<a id="initialization-failures" />
...
<a id="vss-service-monitoring-errors" />
...
<a id="sei-client-and-heartbeat-errors" />

#### 5.4.1 Registration/initialization
<a id="registration-initialization" />
...
<a id="decision-maker-algorithm-errors" />

#### 5.5.1 SDM initialization
<a id="sdm-initialization" />
...
<a id="udp-socket-and-plc-communication" />
...
<a id="software-failure-events-sw-fail" />
...
<a id="sei-daemon-fails-to-start-when-launched-directly" />
...
<a id="safety-ai-monitor-exits-with-baseline-file-not-found" />
<a id="saim-baseline-missing" />
...
<a id="vst-sensor-is-online-but-rtsp-playback-fails" />
<a id="vst-rtsp-playback" />
...
<a id="duplicate-vst-sensor-name" />
...
### 6.6 No Behavior Analytics (BA) events on `mdx-events`
<a id="no-behavior-analytics-ba-events-on-mdx-events" />
<a id="no-ba-events" />
...
<a id="safety-ai-monitor-exits-at-startup-with-a-gpu-decoder-error" />
...
<a id="safety-ai-monitor-exits-with-a-baseline-validation-error" />
...
### 6.9 A single camera stays `SENSOR_INVALID` and does not recover
<a id="a-single-camera-stays-sensor-invalid-and-does-not-recover" />
...
<a id="no-decisions-reach-the-command-receiver-in-sdm-on-fsi-mode" />
...
### 6.11 Events dropped with `Failed to process reported SafetyEvent`
<a id="events-dropped-with-failed-to-process-reported-safetyevent" />
...
Solutions:
   ...
   - Confirm `maxPipelines` in `nvpss.conf` covers every configured
     pipeline ID. See [SEI Configuration](/reference/sei-configuration).

### 6.12 Stale SAIM baselines after the perception stack is redeployed
<a id="stale-saim-baselines-after-the-perception-stack-is-redeployed" />
