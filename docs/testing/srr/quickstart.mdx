...
<Note>
Regression Testing Reporter runs **on top of** an already-deployed SIL stack. It does **not** deploy Halos or VSS — do that first (see [SIL — 2D Perception](/testing/sil/sil-2d)). See [Overview](/testing/srr/overview) for what the Regression Testing Reporter is and what a run produces.
</Note>
...
- [SIL — 2D Perception](/testing/sil/sil-2d) — manual SIL bring-up, or
- [Agentic Deployment](/getting-started/agentic-deployment) — agent-driven bring-up.
...
Re-run it after cloning on a new machine, or after regenerating behavior trees. See [Test Scenarios](/testing/srr/scenarios) for the scenario fixtures and how to regenerate them.
...
See [Test Scenarios](/testing/srr/scenarios) for full details on each case.
...
For the full metric reference, drill-down workflow, and how to read a per-clip report, see [Interpreting the Report](/testing/srr/interpret-results).
...
Tear down the SIL stack (Halos + VSS) via the deploy skills or [SIL — 2D Perception](/testing/sil/sil-2d) — not here.
...
If PASS/FAIL alternates in step with the forklift entering the trailer, this is the known Safety Core counter-drift signature (fix planned upstream). Restart the Halos compose between runs; don't file it as a new regression. See [Interpreting the Report](/testing/srr/interpret-results) for the full signature.
