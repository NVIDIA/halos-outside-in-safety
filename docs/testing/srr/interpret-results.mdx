...
description: ""
---
A single Regression Testing Reporter run produces a tree of graded reports. This page explains what each file contains, what every headline metric means, and how to drill into a failing clip. Run a test first — see [Quick Start Guide](/testing/srr/quickstart).
...
A `PERCEPTION_LIKELY_DEAD` banner in `summary.md` means perception was dead during the run, so `match%` is untrustworthy. Fix perception and re-run — see [Quick Start Guide](/testing/srr/quickstart) Troubleshooting.
...
- [Test Scenarios](/testing/srr/scenarios) — what each scenario was designed to stress.
