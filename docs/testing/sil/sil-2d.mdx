...
For an automated bring-up with a coding agent (Claude Code, Codex, Cursor, and similar), use the `hoisa-deploy-profile` skill — it replaces Steps 1–6 below with a single agent prompt. See [Agentic Deployment](/getting-started/agentic-deployment).
...
Nothing to change. 2D runs the RT-DETR R50 model that ships with `warehouse-2d-app`, and the shipped `warehouse-loading-dock-3cams-synthetic` calibration is used as-is. (3D is different — it needs the R101 Sparse4D model plus a `config.yaml`; see [SIL — 3D Perception](/testing/sil/sil-3d).)
...
- [Quick Start Guide](/testing/srr/quickstart) — Once SIL is healthy, run the **Regression Testing Reporter** to score the safety decision
