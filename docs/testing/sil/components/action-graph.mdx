...
On a normal Halos run the flag is absent and this module is never imported, so there is zero effect on the default pipeline. For how the ground truth is consumed, see [Architecture](/testing/srr/architecture).
...
- [Isaac Sim Configuration](/testing/sil/components/isaac-sim) — scene, cameras, and motion authoring
...
- [Architecture](/testing/srr/architecture) — how `/gt/*` ground truth is scored
