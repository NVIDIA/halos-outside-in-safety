...
See [Prerequisites](/getting-started/prerequisites) for hardware and software requirements.

## Deploy with Halos Deployment Skill (recommended)

For an automated bring-up with a coding agent, use the `hoisa-deploy-profile` skill with the `hil` profile — see [Agentic Deployment](/getting-started/agentic-deployment). The skill drives one host at a time; there is no single command that deploys both hosts in this release.
...
Copy the template unedited: the profile ships `PSF_LAUNCH_MODE=skip`, under which the file is checked only for existence. Leave that line in place — an unset value selects `active`, which reads the file. See [HIL Architecture](/testing/hil/architecture) for what the two modes change.
...
Deploy VSS Warehouse on the safety host following [SIL — 2D Perception](/testing/sil/sil-2d) Step 2, with four changes for HIL. Apply all four **before** the first `up`:
...
- **Calibration**: the 3D profile needs the camera calibration and `camera_info.json` machinery. Follow [SIL — 3D Perception](/testing/sil/sil-3d) for it and for the rest of the VSS-side configuration.
...
ROS 2 discovery: the three x86 containers cannot see each other. [HIL Architecture](/testing/hil/architecture) explains why `LOCALHOST` scoping fails on a multi-interface host.
...
For the Safety Core itself, see [Deployment Guide](/deployment#safety-core-troubleshooting) in the [Deployment Guide ](/deployment). For IGX Thor container runtime and image pull workarounds, see [Annex A](/getting-started/inference-pipeline#annex-a-vss-warehouse-blueprint-deployment-workarounds-for-safety-flashed-igx-thor).

## Next Steps

- [HIL Architecture](/testing/hil/architecture) — Ports, packet format, and the configuration pairing between the two hosts
- [Isaac Sim Configuration](/testing/sil/components/isaac-sim) — Scenario configuration and `run_sdg.sh` options
- [Deployment Guide](/deployment) — Safety Core installation, options, and troubleshooting in full
