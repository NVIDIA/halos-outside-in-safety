...
- **Fresh state**: on an IGX Thor that has run VSS Warehouse before, clear the Kafka volumes first (see [Cleanup and Teardown](#cleanup-and-teardown)), or the sensor distribution service replays old sensor history and perception stays at 0 fps.
...
Every `Sending decision command` line must be followed by a `Received acknowledgment` line carrying the same sequence number, and the destination must be the x86 host on `12346`. Commands sent with nothing coming back means the communication layer is not receiving — see [The Communication Layer Receives Nothing](#the-communication-layer-receives-nothing). No decisions at all means perception is not producing events, so re-check Step 8.
...
2. x86: stop the driver and [start it again](/testing/hil/quickstart#71-start-the-driver), still without `--enable-vst`.
3. x86: re-run the [frames gate](/testing/hil/quickstart#72-confirm-the-streams-deliver-frames).
...
- An Isaac Sim restart wedged the mounts. Follow [Restart the Scenario](#restart-the-scenario) in full.
- The IGX Thor is replaying stale sensor history from an earlier deployment. If it recurs after every restart, remove the Kafka volumes as described in [Cleanup and Teardown](#cleanup-and-teardown).
