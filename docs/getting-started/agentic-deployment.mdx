...
description: ""
---
Deploy Halos Outside-In Safety by **profile**, driven from natural language by a coding agent and the `hoisa-deploy-profile` skill instead of running the steps yourself. If no agent is available, the manual route for each profile is under `Fallback: manual bring-up`_.
