...
how `behavior-trees/` and `navmesh.json` plug into the SIL compose stack at run
...
2. **Make sure the scenario's behavior trees are in the SIL configs dir** —
...
3. **Select the scenario** — copy its three trees onto the fixed active names the
   ...
   ```bash
   for i in 0 1 2; do
     cp -f ${HOISA_ROOT_PATH}/closed-loop-testing/isaac-sim/sil/configs/srr_in-roi_char${i}.bt.json \
           ${HOISA_ROOT_PATH}/closed-loop-testing/isaac-sim/sil/configs/srr_char${i}.bt.json
   done
   ```
4. **Restart Isaac Sim** so it picks up the new trees (compose down/up).
...
`ZONE_CHAR0/1/2` polygons are defined as constants in `randomize_paths.py`
(per-character spawn / wander zones). Update them in the script if the scene
layout changes.
...
These scenarios target the stock warehouse scene
`indicator_warehouse_20x20_layout_overflow_test.usd`. Its baked NavMesh already
covers the three character zones the scenarios exercise, so SRR uses it as-is —
no scene edits, no re-bake. `scenes/navmesh.json` is the export of that scene's
NavMesh (input to `randomize_paths.py`).

If you switch scenes, regenerate ALL behavior trees — waypoints baked into the trees are scene-specific.

---

## Scene structure (`indicator_warehouse_20x20_layout_overflow_test.usd`)
...
### Character zones (`randomize_paths.py` constants)

`ZONE_CHAR0`, `ZONE_CHAR1`, `ZONE_CHAR2` are simple rectangular polygons — one spawn + wander area per character. They're `randomize_paths.py` constants; to change zones, edit the constants and regenerate all behavior trees.

### Cameras + RTSP

Three fixed cameras feed VSS perception via VST. Camera registration with VST happens automatically via `--enable-vst` on the scenario runner. Stream IDs map to: `Camera`, `Camera_01`, `Camera_02` (look up real IDs via VST `/v1/record/streams`).

### What changes between the 5 scenarios

**Only the three active behavior trees** (`srr_char{0,1,2}.bt.json`) in `${HOISA_ROOT_PATH}/closed-loop-testing/isaac-sim/sil/configs/`. Everything else (scene file, NavMesh, ROI calibration, tripwire, forklift/ROS control, cameras) stays identical. The skill / runner copy the selected scenario's `srr_<name>_char{0,1,2}.bt.json` onto those fixed names — see [`scripts/run_multi.sh`](../scripts/run_multi.sh) `phase_set_behavior_tree()`.

---

## End-to-end workflow for a NEW scene

If you're prepping a fresh warehouse USD (not the stock scene), the full sequence is:

```
1. Open the fresh scene in Isaac Sim
2. Run isaac-scripts/scene_prep/disable_exclude_root_layer.py    (Script Editor — only if the scene has NavMesh exclude volumes over the work zones)
3. Run isaac-scripts/scene_prep/clear_navmesh_blockers.py        (Script Editor — only if obstacles overlap the work zones)
...
5. Run isaac-scripts/navmesh/export_navmesh.py                   (Script Editor — writes navmesh.json)
   ── /gt/* publishers are NOT baked into the scene (IRA 6.0 spawns chars at
      runtime, so the ManRoot targets don't exist at save time). They're built at
      RUN time: run_multi.sh launches Isaac with `--srr-gt`, which builds
      action_graphs/srr_ground_truth.py's /World/SRRGraph once the chars spawn.
      Nothing to paste/save. Use srr_pubs/check_srr_prereqs.py (Script Editor,
      while running) only to debug if /gt looks wrong.
6. Copy the new navmesh.json into scenes/:
    cp /path/to/navmesh.json scenarios/scenes/navmesh.json
7. On host, regenerate behavior trees:
    python3 scenarios/tools/randomize_paths.py \
        --navmesh scenarios/scenes/navmesh.json \
