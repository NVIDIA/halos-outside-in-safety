...
Deployment is long-running (NGC image pulls, perception engine build, Isaac Sim shader compile), so use a high-context model with maximum effort, and expect the agent to pause for host-level input it cannot do for you: `sudo` / system installs, the NGC API key, and the GPU / profile choice. "Complete" means sim-driven MUTE / UNMUTE transitions appear in the logs, not merely that all containers are up. The host prerequisites are yours to provide (NGC access to `nvidia/outside-in-safety`, Docker, and the Isaac Sim GPU); see [`hoisa-deploy-profile/references/prerequisites.md`](hoisa-deploy-profile/references/prerequisites.md).
